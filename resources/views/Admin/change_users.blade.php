    ...
    <section class="orders">
        ...
        <div class="orders-table">
            <table class="orders-table-content">
                ...
                @if(count($users) > 0)
                    <tbody>
                    @foreach($users as $orderItem)
                        <tr>
                            <td class="orders-table-data">

                                {{$orderItem->id}}
                            </td>
                            <td class="orders-table-data">{{$orderItem->name}}</td>
                            <td class="orders-table-data">{{$orderItem->phone}}</td>
                            <td class="orders-table-data">{{$orderItem->email}}</td>
                            <td class="orders-table-data">{{$orderItem->usertype}}</td>
                            <td class="orders-table-data">
                                <i class="fa fa-edit fa-lg" style="color: blue; cursor: pointer;" data-bs-toggle="modal" data-bs-target="#exampleModal_{{$orderItem->id}}"></i>
                                <i class="fa fa-trash fa-lg" style="color: red; cursor: pointer;" data-bs-toggle="modal" data-bs-target="#deleteUserModal_{{$orderItem->id}}"></i>

                                <!-- Modal -->
                                <div class="modal fade" id="exampleModal_{{$orderItem->id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                <form action="{{route('changeUser', ['id'=>$orderItem->id])}}" method="post" enctype="multipart/form-data">
                                                    @csrf
                                                    <div class="mb-3 row">
...
{{--                                Modal to delete users--}}
                                <!-- Modal -->
                                <div class="modal fade" id="deleteUserModal_{{$orderItem->id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h1 class="modal-title fs-5" id="exampleModalLabel">Modal title</h1>
                                            ...
                                            <div class="modal-body">
                                                <form action="{{route('deleteUser', ['id'=>$orderItem->id])}}" method="post" enctype="multipart/form-data">
                                                    @csrf
